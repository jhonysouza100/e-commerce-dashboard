...
export default function OrdersManager() {
  ...
  if (selectedOrderId !== null) {
    if (detailQuery.isLoading)
      return (
        <div className="rounded-md bg-background p-5 shadow-md">
          <Loading message="orden..." />
        </div>
      );
    if (detailQuery.isError)
      return (
        <div className="rounded-md bg-background p-5 shadow-md">
          <Alert message={detailQuery.error.message} />
        </div>
      );
    if (detailQuery.data)
      return (
  ...
  const count: OrdersPagination = listQuery.data?.count || {
    page: 1,
    limit: 6,
    total: 0,
    totalPages: 0,
